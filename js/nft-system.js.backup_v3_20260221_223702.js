/**
 * NFT Horse System v2.0
 * 도트경마 v5.2 - 새로운 NFT 말 시스템
 * 랜덤 박스 4종, 100만가지 이름, 무료 브론즈 박스
 */

const NFT_BOXES = {
  BRONZE: {
    name: '브론즈',
    price: 10, // 10 DOT (原价 10,000원)
    probabilities: { COMMON: 0.90, RARE: 0.10, EPIC: 0, LEGENDARY: 0 },
    color: '#cd7f32',
    freePerDay: 1
  },
  SILVER: {
    name: '실버',
    price: 50, // 50 DOT (原价 50,000원)
    probabilities: { COMMON: 0.70, RARE: 0.25, EPIC: 0.05, LEGENDARY: 0 },
    color: '#c0c0c0',
    freePerDay: 0
  },
  GOLD: {
    name: '골드',
    price: 200, // 200 DOT (原价 200,000원)
    probabilities: { COMMON: 0.50, RARE: 0.35, EPIC: 0.14, LEGENDARY: 0.01 },
    color: '#ffd700',
    freePerDay: 0
  },
  DIAMOND: {
    name: '다이아몬드',
    price: 1000, // 1000 DOT (原价 1,000,000원)
    probabilities: { COMMON: 0, RARE: 0.40, EPIC: 0.50, LEGENDARY: 0.10 },
    color: '#b9f2ff',
    freePerDay: 0
  }
};
...
const NFT_RARITIES = {
  COMMON: { name: '일반', color: '#95A5A6', buffRatio: 0.05, minStat: 40, maxStat: 60 },
  RARE: { name: '레어', color: '#3498DB', buffRatio: 0.08, minStat: 60, maxStat: 75 },
  EPIC: { name: '에픽', color: '#9B59B6', buffRatio: 0.12, minStat: 75, maxStat: 85 },
  LEGENDARY: { name: '전설', color: '#FFD700', buffRatio: 0.20, minStat: 85, maxStat: 100 }
};

const NFT_PREFIXES = ['신속', '빠른', '강력', '용감', '희망', '승리', '천둥', '폭풍', '불꽃', '백색', '흑색', '적색', '청색', '황금', '은빛', '보석', '미친', '완벽', '传奇', '무적'];
const NFT_SUFFIXES = ['질주', '질풍', '疾風', '電光', 'ieger', '_runner', '경마', '마왕', '대박', '복권', '억소리', '대박러시', '슈퍼', '울트라', '메가', '기가', '테라', '�타', '엑사', '존'];
...
class NFTSystem {
  ...
  generate100万Name(id) {
    ...
    return NFT_PREFIXES[prefixIndex] + NFT_SUFFIXES[suffixIndex] + '#' + num;
  }
  ...
  getRandomRarity(boxType) {
    const box = NFT_BOXES[boxType];
    const rand = Math.random();
    let cumulative = 0;
    
    for (const [key, prob] of Object.entries(box.probabilities)) {
      cumulative += prob;
      if (rand < cumulative) {
        return { key, ...NFT_RARITIES[key] };
      }
    }
  ...
  async createNFT(boxType = 'BRONZE', isFree = false) {
    ...
    const rarity = this.getRandomRarity(boxType);
    const nftId = 'nft_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    
    const nft = {
      id: nftId,
      type: 'HORSE',
      name: this.generate100万Name(nftId),
      rarity: rarity,
    ...
    if (this.checkFirebaseReady()) {
      try {
        ...
        console.log('[NFT] Firebase 저장 실패, localStorage만 사용');
      }
    }
  ...
  async generateNFTImage(nft) {
    return new Promise(resolve => {
      ...
      gradient.addColorStop(0, nft.color);
      gradient.addColorStop(1, this.darkenColor(nft.color, 30));
      ...
      this.drawSVGHorse(ctx, 100, 120, nft.color, 1.2);
      ...
      const rColor = rarityColors[nft.rarity.key] || '#95A5A6';

      ctx.fillStyle = rColor;
      ctx.fillRect(0, 0, 200, 8);
      ctx.fillRect(0, 192, 200, 8);

      ctx.font = 'bold 14px Arial';
      ctx.fillStyle = '#FFFFFF';
      ctx.textAlign = 'center';
      ctx.fillText(nft.name, 100, 175);

      ctx.font = 'bold 12px Arial';
      const rarityText = nft.rarity.key === 'LEGENDARY' ? '★전설★' :
                         nft.rarity.key === 'EPIC' ? '♦에픽♦' :
                         nft.rarity.key === 'RARE' ? '★레어★' : '일반';
      ctx.fillText(rarityText, 100, 192);

      resolve(canvas.toDataURL('image/png'));
    });
  }

  drawSVGHorse(ctx, x, y, color, scale) {
    ctx.save();
    ctx.translate(x, y);
    ...
    ctx.beginPath();
    ctx.ellipse(-20, -5, 12, 8, -0.3, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.moveTo(20, -10);
    ctx.quadraticCurveTo(35, -15, 40, -5);
    ctx.quadraticCurveTo(35, 0, 25, 5);
    ctx.fill();
    ...
    ctx.fillStyle = '#000';
    ctx.beginPath();
    ctx.arc(-25, -9, 1.5, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = '#FFE4B5';
    ctx.fillRect(-5, 8, 4, 15);
    ctx.fillRect(5, 8, 4, 15);
    ctx.fillRect(-15, 8, 4, 12);
  ...
  convertToMoney(nftId) {
    ...
    const [minPrice, maxPrice] = nft.rarity.key === 'LEGENDARY' ? [800000, 2000000] :
                                  nft.rarity.key === 'EPIC' ? [150000, 400000] :
                                  nft.rarity.key === 'RARE' ? [40000, 100000] : [8000, 20000];
...
async function initNFTSystem() {
  ...
  console.log('[NFT System v2.0] 초기화 완료');
...
function renderBoxCard(boxType) {
  ...
  return `
    <div style="background: linear-gradient(135deg, ${box.color}22, ${box.color}44); border: 2px solid ${box.color}; border-radius: 12px; padding: 15px; text-align: center; cursor: pointer; transition: transform 0.2s;" onclick="openBox('${boxType}')">
      ...
      <div style="font-size: 11px; color: #666; margin-top: 3px;">${getBoxProbText(boxType)}</div>
    </div>
  `;
}
...
function getBoxProbText(boxType) {
  const texts = {
    BRONZE: '일반 90%, 레어 10%',
    SILVER: '일반 70%, 레어 25%, 에픽 5%',
    GOLD: '일반 50%, 레어 35%, 에픽 14%, 전설 1%',
    DIAMOND: '레어 40%, 에픽 50%, 전설 10%'
  };
  return texts[boxType] || '';
}
...
async function openBox(boxType, isFree = false) {
  ...
  showBoxOpeningAnimation(boxType, async () => {
    try {
      const nft = await nftSystem.createNFT(boxType, isFree);
      
      if (isFree) {
        nftSystem.useFreeBox();
      }
      
      showNFTOpenResult(nft);
      renderNFTTab();
    } catch (err) {
...
function showNFTOpenResult(nft) {
  ...
  const rarityColors = {
    LEGENDARY: 'linear-gradient(135deg, #FFD700, #FF6B00)',
    EPIC: 'linear-gradient(135deg, #9B59B6, #E74C3C)',
    RARE: 'linear-gradient(135deg, #3498DB, #2ECC71)',
  ...
  modal.innerHTML = `
    <div style="background: ${rarityColors[nft.rarity.key] || rarityColors.COMMON}; border-radius: 20px; padding: 30px; text-align: center; max-width: 350px; animation: popIn 0.5s;">
      <style>
        @keyframes popIn {
...
function renderNFTCard(nft) {
  ...
  return `
    <div style="background: #1a1a2e; border: 2px solid ${rarityColors[nft.rarity.key] || '#333'}; border-radius: 10px; padding: 10px; ${isSelected ? 'box-shadow: 0 0 15px ' + rarityColors[nft.rarity.key] : ''}">
      <div style="background: ${nft.color}; border-radius: 8px; height: 80px; display: flex; align-items: center; justify-content: center; font-size: 40px;">
        🐴
      </div>
      <div style="font-size: 12px; margin-top: 8px; color: ${rarityColors[nft.rarity.key]}; font-weight: bold;">${nft.rarity.name}</div>
    ...
    return `
      <div style="background: #1a1a2e; padding: 10px; margin: 5px 0; border-radius: 8px; display: flex; align-items: center;">
        ...
        <div style="flex: 1; margin-left: 10px;">
          ...
          <div style="color: ${NFT_RARITIES[nft.rarity.key]?.color || '#888'}; font-size: 11px;">${nft.rarity.name}</div>
        </div>
        <div style="text-align: right;">
          <div style="color: #9df7c7; font-size: 12px; font-weight: bold;">≈${avgPrice} DOT</div>
...
function selectNFTForRace(nftId) {
  ...
  if (nft) {
    alert(`${nft.name} 말을 경주에 사용합니다!\n버프: 속도 +${nft.stats.speed * nft.rarity.buffRatio}, 지구력 +${nft.stats.stamina * nft.rarity.buffRatio}, 스퍼트 +${nft.stats.burst * nft.rarity.buffRatio}`);
    renderNFTInventory();
  }
}
...
function showConvertModal(nftId) {
  ...
  if (!confirm(`${nft.name} (${nft.rarity.name})을(를) DOT로 환전하시겠습니까?\n\n예상 수령금: ${actual} DOT\n(수수료 10%: ${fee} DOT)`)) {
    return;
  }
  
  const result = nftSystem.convertToMoney(nftId);
  if (result.success) {
    alert(`${result.amount} DOT가 지갑에 추가되었습니다!`);
    renderNFTTab();
  }
}
...
console.log('[NFT System v2.0] 모듈 로드 완료');
