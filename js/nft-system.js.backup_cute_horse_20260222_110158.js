...
class NFTSystem {
  ...
  async generateNFTImage(nft) {
    return new Promise(resolve => {
      ...
      ctx.fillText(nft.name, 100, 172);

      ctx.font = 'bold 11px Arial';
      const genderText = nft.gender === '수' ? '♂수' : '♀암';
      const bloodlineText = nft.bloodline ? '血' : '';
      ctx.fillText(genderText + bloodlineText, 100, 188);

      if (nft.glowLevel !== 'none') {
        ctx.font = 'bold 10px Arial';
        const glowText = nft.glowLevel === 'ultimate' ? '✨신화✨' :
                        nft.glowLevel === 'enhanced' ? '★전설★' : '☆영웅☆';
        ctx.fillStyle = nft.rarity.color;
        ctx.fillText(glowText, 100, 25);
      }
...
async function openBox(boxType, isFree = false) {
  ...
  showBoxOpeningAnimation(boxType, async () => {
    try {
      const result = await nftSystem.createNFT(boxType, isFree);
