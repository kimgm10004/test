...
(function() {
  ...
  const PHYSICS = {
    ...
    // 스타일 가중치
    STYLE_WEIGHTS: {
      '선행': { early: 1.15, mid: 1.0, late: 0.85 },
      '선입': { early: 1.05, mid: 1.1, late: 0.9 },
      '추입': { early: 0.85, mid: 0.95, late: 1.25 },
      '차입': { early: 0.9, mid: 1.0, late: 1.15 },
      '만능': { early: 1.0, mid: 1.0, late: 1.0 }
    }
  };
  ...
  class PhysicsEngine {
    ...
    calculateVelocity(state, progress, weather) {
      ...
      // 최종 속도 계산
      let velocity = maxSpeed * phaseMultiplier * fatigue * burst * weatherFactor * condFactor * synergyBonus * randomFactor * stumbleFactor;
      
      // 속도 제한
      velocity = MU.clamp(velocity, PHYSICS.MIN_VELOCITY, PHYSICS.MAX_VELOCITY * 1.2);
      
      // 랜덤 말림 발생 (약 1% 확률)
      if (Math.random() < 0.008 && state.stumble === 0 && progress > 0.1 && progress < 0.9) {
        state.stumble = 30 + Math.floor(Math.random() * 40);
      }
